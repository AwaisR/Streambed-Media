...
function Collaborator(props) {
  ...
  const verifyCollaborator = async () => {
    ...
    await axios
      .post(
        "/api/instagram/verify-collaborator",
  ...
  return (
    <div className="upload-wrap collaborators-new">
      <div className="upload-platforms">
        {instagramPost.success ? (
          <InstagramOverView
            p_id={instagramPost.post.post_id}
            collaborator={videoCollaborators}
            title={instagramPost.post.text || "To be added"}
            txid={txid}
            post={instagramPost.post}
            sia_url={instagramPost.post.sia_url}
          />
        ) : (
