...
function Collaborator(props) {
  ...
  const { vid, _id } = props.location.state.FB_DOC;
  try {
    if (!props.location.state?.facebook) {
      history.push("/");
    }
  } catch (error) {}
  ...
  return (
    <div className="upload-wrap collaborators-new">
      <div className="upload-platforms">
        {facebookPost.success ? (
          <UploadOverView
            p_id={facebookPost.post.post_id}
            collaborator={videoCollaborators}
            media_url={facebookPost.post.media_url}
            title={facebookPost.post.message}
            txid={txid}
            plateform="facebook"
            sia_url={facebookPost.post.sia_url}
          />
        ) : (
